...
public class EquipmentWOController {
	...
	@ResponseBody
	public Response addSIHistoryWOInfo(@RequestBody SIHistoricalBean historicalBean) {
		
		SIHistoryWO siHistoryWO = new SIHistoryWO();
		...
		try {
			created_date = df.parse(modifiedDate);
		...
		Date lastupdated_date = null;
		try {
			lastupdated_date = df.parse(modifiedDate2);
		} catch (ParseException e) {
			
			e.printStackTrace();
		}
		
		siHistoryWO.setWorkorder_id(historicalBean.getWorkorder_id());
		siHistoryWO.setAnomaly(historicalBean.getAnomaly());
		siHistoryWO.setMembers(historicalBean.getMembers());
		siHistoryWO.setSection(historicalBean.getSection());
		siHistoryWO.setDates(historicalBean.getDates());
		siHistoryWO.setCreatedby("GE");
		siHistoryWO.setCreated_date(created_date);
		siHistoryWO.setLastupdated_by("502647982");
		siHistoryWO.setLastupdated_date(lastupdated_date);
		
		equipService.addSIHistoryWOInfo(siHistoryWO);
		
		FiveWhyDetails fiveWhyDetails = new FiveWhyDetails();
				
		fiveWhyDetails.setType(historicalBean.getType());
		fiveWhyDetails.setAttempt1(historicalBean.getAttempt1());
		fiveWhyDetails.setAttempt2(historicalBean.getAttempt2());
		fiveWhyDetails.setAttempt3(historicalBean.getAttempt3());
		fiveWhyDetails.setAttempt4(historicalBean.getAttempt4());
		fiveWhyDetails.setAttempt5(historicalBean.getAttempt5());
		fiveWhyDetails.setSuggestof_imp(historicalBean.getSuggestof_imp());
		fiveWhyDetails.setCreated_by("CAPGEMINI");
		fiveWhyDetails.setCreated_date(created_date);
		fiveWhyDetails.setLastupdated_by("502647982");
		fiveWhyDetails.setLastupdated_date(lastupdated_date);

		equipService.editSIHistoryWOInfo(fiveWhyDetails);
	...
	@ResponseBody
	public Response addMaintReportInfo(@RequestBody MaintainanceReportBean maintainanceBean) {
		
		MaintenanceReportHdr maintenanceReportHdr = new MaintenanceReportHdr();
		
		Date date = new Date();
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		String modifiedDate = new SimpleDateFormat("yyyy-MM-dd").format(date);
		Date receivedRepairTime = null;
		try {
			receivedRepairTime = df.parse(modifiedDate);
		} catch (ParseException e) {
		...
		try {
			startRepairTime = df.parse(modifiedDate2);
		} catch (ParseException e) {
		...
		String modifiedDate3 = new SimpleDateFormat("yyyy-MM-dd").format(date3);
		Date startRepresentativeTime = null;
		try {
			startRepresentativeTime = df.parse(modifiedDate3);
		} catch (ParseException e) {
			
			e.printStackTrace();
		}
		
		maintenanceReportHdr.setOperator(maintainanceBean.getOperator());
		maintenanceReportHdr.setFailureTime(maintainanceBean.getFailureTime());
		maintenanceReportHdr.setWorkshop(maintainanceBean.getWorkshop());
		maintenanceReportHdr.setEndMaintStaff(maintainanceBean.getEndMaintStaff());
		maintenanceReportHdr.setStartMaintStaffSSOId(maintainanceBean.getStartMaintStaffSSOId());
		maintenanceReportHdr.setReceivedRepairTime(maintainanceBean.getReceivedRepairTime());
		maintenanceReportHdr.setStartRepairTime(maintainanceBean.getStartRepairTime());
		maintenanceReportHdr.setStartRepresentativeTime(maintainanceBean.getStartRepresentativeTime());
		...
		maintenanceReportDetail.setPositionNumber(maintainanceBean.getPositionNumber());*/
    ...
    for(int i=0;i<listreport.size();i++) {
	    ...
	    format1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS",Locale.ENGLISH).format(listreport.get(i).getCreatedDate());
	    table.addCell(format1);
	    format2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS",Locale.ENGLISH).format(listreport.get(i).getLastUpdatedDate());

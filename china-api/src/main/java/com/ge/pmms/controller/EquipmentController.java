...
public class EquipmentController {
	...
	@ResponseBody
	List<EquipmentInfo> getAllEquipDetails(@RequestParam(required = false) String equipCategory) {
		List<EquipmentInfo> ls = equipService.getAllEquipDetails(equipCategory);
		return ls;
	}
	
	//test
	@RequestMapping(value = "/getEquipDetails", method = RequestMethod.GET)
	@ResponseBody
	List<EquipmentInfo> getEquipDetails(@RequestParam String equipId) {
		List<EquipmentInfo> ls = equipService.getEquipInfoOnId(equipId);
		return ls;
	}
	...
	public void downloadExcel(HSSFWorkbook workbook, HttpServletRequest request,HttpServletResponse response) throws IOException{
	    ...
	    for(int i=0;i<lstequipmentinfo.size();i++) {
	      ...
	      //  aRow.createCell(8).setCellValue(lstequipmentinfo.get(i).getReleaseDate());
	        ...
	        if(lstequipmentinfo.get(i).getDeptId2()!=null)
